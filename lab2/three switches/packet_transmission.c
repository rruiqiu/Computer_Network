...
long
schedule_end_packet_transmission_event2(Simulation_Run_Ptr simulation_run,
	double event_time,
	Server_Ptr link)
{
	Event event;
...
//packet2 copies
void
end_packet_transmission_event2(Simulation_Run_Ptr simulation_run, void* link)
{
	Simulation_Run_Data_Ptr data;
	Packet_Ptr this_packet, next_packet;
	...
	//will need to add more statements here to determine where this packet is coming from, so originate from either 1 or 2 or 3.
	...
	//data->accumulated_delay += data->accumulated_delay2;
	/* Output activity blip every so often. */
	output_progress_msg_to_screen(simulation_run);
...
//packet2 copies
void
end_packet_transmission_event3(Simulation_Run_Ptr simulation_run, void* link)
{
	Simulation_Run_Data_Ptr data;
	Packet_Ptr this_packet, next_packet;
...
double
get_packet_transmission_time(long int bitrate)
{
  return ((double) PACKET_LENGTH/bitrate);
}
