...
output_results(Simulation_Run_Ptr simulation_run, FILE * writeFile)
{
  ...
  printf("Transmitted packet count on 1 = %ld (Service Fraction = %.5f)\n",
	 data->number_of_packets_processed, xmtted_fraction);

  printf("Transmitted packet count on 2 = %ld (Service Fraction = %.5f)\n",
      data->number_of_packets_processed2, xmtted_fraction);

  printf("Transmitted packet count on 3 = %ld (Service Fraction = %.5f)\n",
      data->number_of_packets_processed3, xmtted_fraction);

  printf("Arrival rate = %.3f packets/second \n", (double) data->arrival_rate);
  
  printf("Arrival rate2 = %.3f packets/second \n", (double)data->arrival_rate2);
  ...
  printf("The current pb is %f\n", data->pb);
  printf("\n");

  fprintf(writeFile, "Mean Delay 1 = %10.2f, Mean Delay 2 (msec) = %10.2f,Mean Delay 3 (msec) = %10.2f, The current pb is %4.2f\n", 1e3 * data->accumulated_delay / data->number_of_packets_processed, 1e3 * data->accumulated_delay2 / data->number_of_packets_processed2, 1e3 * data->accumulated_delay3 / data->number_of_packets_processed3, data->pb);
}
