...
void output_results(Simulation_Run_Ptr this_simulation_run, FILE * writefile)
{
  ...
  printf("The left customer is = %ld \n", sim_data->call_left);
  printf("prob taxi arrive and customer left = %f\n", sim_data->call_left / (double) (sim_data->call_arrival_count -
      sim_data->blocked_call_count));
  fprintf(writefile, "Blocking probability = %.5f, Taxi left = %7ld, prob of taxi arrving and customer left = %.5f  Num channel is = %d \n", 1 - xmtted_fraction, sim_data->call_left, sim_data->call_left / (double) (sim_data->call_arrival_count -
      sim_data->blocked_call_count), sim_data->num_channel);
  printf("\n");
}
