...
int main(void)
{
  ...
  while ((random_seed = RANDOM_SEEDS[j++]) != 0) {
    for (data.number_of_channels = 1; data.number_of_channels <= 20;data.number_of_channels++)
    {
      ...
      /* Create the channels. */
      data.channels = (Channel_Ptr *) xcalloc((int) data.number_of_channels,
                sizeof(Channel_Ptr));

      data.buffer = fifoqueue_new();
      /* Initialize the channels. */
      for (i=0; i<data.number_of_channels; i++) {
